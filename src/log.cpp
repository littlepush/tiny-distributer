...
inline bool td_log_status() {
	unique_lock<mutex> _l(g_logmutex);
	return g_logstatus;
}

inline const char *td_log_lv_to_string(td_log_level lv) {
...
inline void td_log_get_time(string & logline) {
...
void td_start_log_thread( ) {
	if ( g_logthread != NULL ) return;
	g_logstatus = true;
	g_logthread = new thread(td_log_write_worker);
}
...
// Log to syslog
void td_log_start(td_log_level lv) {
...
void td_log(td_log_level lv, const char *format, ...) {
	...
	string _logline;
	if ( g_logsys == false ) {
		td_log_get_time(_logline);
		_logline += "[";
		_logline += td_log_lv_to_string(lv);
		_logline += "] ";
	}
